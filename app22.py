...
def main():
    ...
    if not st.session_state.authenticated:
        ...
        st.write(f"**Usuario:** {st.session_state.current_user} (**Rol:** {st.session_state.role})")
...
def show_loto_section():
    ...
    # 2) Registrar Candado (Clásico)
    with sub_tabs[1]:
        if st.session_state.role in ["admin", "operador"]:
            input_data()
        else:
            st.error("No tienes permiso para registrar candados (se requiere rol operador o admin).")

    # 3) Registrar Candado (Dinámico) - NUEVO
    with sub_tabs[2]:
        if st.session_state.role in ["admin", "operador"]:
    ...
    with sub_tabs[3]:
        if st.session_state.role == "admin":
            ...
            st.error("Sólo un admin puede editar/borrar candados.")
    ...
    with sub_tabs[4]:
        if st.session_state.role in ["admin", "operador"]:
            ...
            st.error("No tienes permiso para generar reportes (se requiere rol operador o admin).")
    ...
    with sub_tabs[5]:
        if st.session_state.role == "admin":
            ...
            st.error("Sólo un admin puede administrar usuarios.")
...
def show_dashboard():
    ...
    # Listado
    st.markdown("### Listado de Candados (recientes)")
    df_sorted = df.copy()
    if "Fecha" in df_sorted.columns:
        df_sorted["Fecha"] = pd.to_datetime(df_sorted["Fecha"], errors="coerce")
...
# --------------------------------------------------------------------------------
# LOTO - REGISTRAR CANDADO (CLÁSICO)
# --------------------------------------------------------------------------------
def input_data():
    st.markdown("<h1 style='text-align:center; color:#4dd0e1;'>Registrar Nuevo Candado (Clásico)</h1>",
                unsafe_allow_html=True)
...
# --------------------------------------------------------------------------------
# LOTO - REGISTRAR CANDADO (DINÁMICO) - NUEVO
# --------------------------------------------------------------------------------
def registrar_candado_dinamico():
    """
    Registrar candado a partir de un Excel con columnas: field_type, label, default, options.
    Genera un PDF con QR.
    """
    st.markdown("<h1 style='text-align:center; color:#4dd0e1;'>Registrar Nuevo Candado (Dinámico)</h1>", 
                unsafe_allow_html=True)

    # Subir Excel
    form_excel = st.file_uploader("Subir Excel para definir campos (field_type, label, default, options)",
                                  type=["xlsx", "xls"])
    if not form_excel:
        st.info("Sube un archivo Excel para generar el formulario.")
        return
    ...
    st.write("Completa el formulario. Los campos provienen del Excel subido.")
    form_values = {}
    ...
    if submitted:
        ...
        st.success("Registro guardado exitosamente (dinámico).")

        # Generar PDF con QR
        candado_id = form_values["ID"]
        link_qr = f"http://miapp.com/loto?id={candado_id}"  # Ajusta a tu URL real
...
def generar_pdf_dinamico_qr(data_dict: dict, link: str) -> bytes:
    ...
    pdf.cell(0, 10, "Registro de Candado (Dinámico)", ln=True, align="C")
    pdf.ln(5)
    ...
    pdf.ln(40)
    pdf.multi_cell(0, 8, f"Escanea el QR: {link}")
...
def edit_or_delete_candado():
    ...
    if df_display.empty:
        st.info("No se encontraron columnas relevantes en el DF.")
        return
    ...
    original_idx = select_idx
    row_data = df.iloc[original_idx]
    ...
    with col_right:
        if st.button("Borrar"):
            df.drop(df.index[original_idx], inplace=True)
            st.session_state["candados_df"] = df
            save_loto_excel(df)
            st.success("Candado borrado.")
            st.experimental_rerun()

    if "edit_mode" in st.session_state and st.session_state["edit_mode"] == original_idx:
        with st.expander(f"Editando ID: {row_data['ID']}", expanded=True):
            edit_candado_form(original_idx)

def edit_candado_form(idx):
    df = st.session_state["candados_df"]
    candado = df.loc[idx]
    with st.form(f"edit_form_{idx}", clear_on_submit=True):
        new_desc = st.text_input("Descripción", value=str(candado["Descripción"]))
        new_resp = st.text_input("Responsable", value=str(candado["Responsable"]))
        new_fecha = st.date_input("Fecha", value=pd.to_datetime(candado["Fecha"]))
        new_estado = st.selectbox("Estado", ["Activo", "Inactivo"], 
                                  index=0 if candado["Estado"] == "Activo" else 1)
...
def show_precomisionado_section():
    st.markdown("<h2 style='text-align:center; color:#4dd0e1;'>Precomisionado - Dossier Digital</h2>", 
                unsafe_allow_html=True)
...
# --------------------------------------------------------------------------------
# SUB-SECCIÓN: LISTA DE ITEMS (Precomisionado)
# --------------------------------------------------------------------------------
def show_item_list():
...
# --------------------------------------------------------------------------------
# SUB-SECCIÓN: GENERAR ITR (PDF) (Precomisionado)
# --------------------------------------------------------------------------------
...
# --------------------------------------------------------------------------------
# FORMULARIO EXCEL DINÁMICO (Precomisionado)
# --------------------------------------------------------------------------------
def run_document_form():
    """
    Ejemplo de formulario dinámico (desde Excel) en la sección Precomisionado.
    """
    st.write("### Crear formulario a partir de un archivo Excel (Demo)")
